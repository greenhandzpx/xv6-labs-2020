...
e1000_transmit(struct mbuf *m)
{
  ...
  //
  acquire(&e1000_lock);
  struct tx_desc *td = &tx_ring[regs[E1000_TDT]];
  if (!(td->status & E1000_TXD_STAT_DD)) {
    // the desc hasn't finished yet
    release(&e1000_lock);
    return -1;
  }
  struct mbuf **tb = &tx_mbufs[regs[E1000_TDT]];
  if (*tb != 0x0) {
    // free the finished buf
    mbuffree(*tb);
  }
  *tb = m;
